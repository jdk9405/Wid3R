...
class BaseTrainer:
    ...
    def prepare_model(self):
        ...
        if self.cfg.model.ckpt is not None and os.path.exists(self.cfg.model.ckpt) and not self.cfg.model.ckpt.endswith('.safetensors'):
            self.log_info(f"Load from model weights at {self.cfg.model.ckpt}")
            state_dict = torch.load(self.cfg.model.ckpt, map_location="cpu")
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for it, batch in enumerate(metric_logger.log_every(
                self.test_loader, self.cfg.test.print_freq, header
            )):
                ...
                outputs = self.calculate_loss(forward_output, mode='test')
                loss = outputs.loss
                ...
                # memory...
                if isinstance(forward_output, list) and len(forward_output) == 2:
                    pred, batch_from_forward = forward_output
                    del pred, batch_from_forward
                del forward_output, batch, outputs, loss
                torch.cuda.empty_cache()  # validation은 더 자주 정리
                if it % 10 == 0:
                    gc.collect()
                    torch.cuda.empty_cache()
                    torch.cuda.reset_peak_memory_stats()
    ...
    def get_scheduler_step(self):
        actual_scheduler = getattr(self.lr_scheduler, 'scheduler', self.lr_scheduler)
        current_step = -1
        if hasattr(actual_scheduler, '_step_count'):
            current_step = actual_scheduler._step_count
        elif hasattr(actual_scheduler, 'last_epoch'):
            current_step = actual_scheduler.last_epoch + 1
        return current_step
    ...
    def train_one_epoch(self, epoch):
        ...
        current_scheduler_step = self.get_scheduler_step()
        ...
        for it, batch in enumerate(metric_logger.log_every(
            ...
            for _ in range(self.cfg.train.gradient_accumulation_steps):
                with self.accelerator.accumulate(self.model):
                    ...
                    batch_output = self.calculate_loss(forward_output, mode="train")
                    ...
                    is_finite_local = torch.isfinite(loss_value)
                    all_finite_across_ranks = self.accelerator.gather_for_metrics(is_finite_local)
                    skip_step = not torch.all(all_finite_across_ranks).item()
                    del all_finite_across_ranks, is_finite_local    # memory

                    if skip_step:
                        rank = get_rank()
                        print(
                            f"Rank {rank}: Loss is {loss_value}, global step {self.global_step}, dataset: {[batch[i]['dataset'][0] for i in range(len(batch))]}.",
                            force=True,
                        )
                        if self.accelerator.state.deepspeed_plugin is None:
                            self.optimizer.zero_grad()

                        # del forward_output, batch
                        del loss, loss_value, batch, forward_output, batch_output
                        torch.cuda.empty_cache()
                        continue
                    else:
                    ...
                    for item in batch_output:
                        if 'loss' in item:
                            gathered = self.accelerator.gather(batch_output[item])
                            batch_output[item] = gathered.mean().item()
                            del gathered
                            if item in loss_details_dict:
                                loss_details_dict[item] += batch_output[item] / self.cfg.train.gradient_accumulation_steps if loss_value.item() != 0 else 0.0
                            else:
                                loss_details_dict[item] = batch_output[item] / self.cfg.train.gradient_accumulation_steps if loss_value.item() != 0 else 0.0
                    ...
                    if self.accelerator.sync_gradients:
                        current_step = self.get_scheduler_step()
                        
                        if current_step < self.cfg.train.lr_scheduler.total_steps:
                            self.lr_scheduler.step()
                        else:
                            if self.accelerator.is_main_process:
                                self.log_info(
                                    "WARNING: Scheduler has reached total_steps ({}/{}), skipping scheduler.step()".format(
                                        current_step,
                                        self.cfg.train.lr_scheduler.total_steps
                                    )
                                )
