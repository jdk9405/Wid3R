...
__HIGH_QUALITY_DATASETS__ = ['TartanAirV2', 'ASE', 'MP3D', 'ScanNetpp', 'STFD', 'Hypersim', 'EDEN', 'vKITTI']
__MIDDLE_QUALITY_DATASETS__ = []
__LOW_QUALITY_DATASETS__ = ['Loc360', 'KITTI360']
...
def create_dataloader(cfg, mode):
    ...
    if isinstance(cfg_dataset, str):
        ...
        augs_dk = None
        if 'augs_dk' in cfg_dataset:
            augs_dk = cfg_dataset.augs_dk
        ...
        if mode == 'train' and 'random_reslution' in cfg.train and cfg.train.random_reslution:
            ...
            for dataset_name, weight in weights.items():
                ...
                dataset_i = hydra.utils.instantiate(cfg_dataset[dataset_name], resolution=(518, 518))
                dataset_i.convert_attributes()
                datasets_all.append(weight @ dataset_i)
