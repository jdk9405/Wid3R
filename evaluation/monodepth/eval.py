...
from evaluation.utils.depth import EVAL_DEPTH_METADATA, depth_evaluation
from evaluation.utils.files import get_all_sequences, list_depths_a_sequence
...
def main(hydra_cfg: DictConfig):
    ...
    for idx_dataset, dataset_name in enumerate(all_eval_datasets, start=1):
        ...
        if dataset_info.type == "video":
            ...
            gt_paths = {seq: list_depths_a_sequence(dataset_info, seq) for seq in seq_list}
            pred_paths = {seq: sorted(glob.glob(f"{output_root}/{seq}/*.npy")) for seq in seq_list}
        ...
        for idx_seq, seq in enumerate(seq_list, start=1):
            ...
            for idx in tqdm(range(len(seq_gt_paths))):
                ...
                # 5. evaluate depth
                depth_results, error_map, depth_predict, depth_gt = depth_evaluation(pred_depth, gt_depth, **depth_evaluation_kwargs)
