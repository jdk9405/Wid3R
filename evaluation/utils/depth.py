...
EVAL_DEPTH_METADATA = {
    "MP3D": {
        "depth_read_func": lambda filename: depth_read_mp3d(filename),
    ...
    "STFD": {
        "depth_read_func": lambda filename: depth_read_stfd(filename),
        "depth_evaluation_kwargs": {
            "max_depth": 10.0,
            "use_gpu": True,
        }
...
def depth_read_mp3d(filename: str):
    # loads depth map D from png file
    # and returns it as a numpy array,
    depth = cv2.imread(filename, cv2.IMREAD_ANYDEPTH).astype(np.float32) / 4000.
    depth = cv2.resize(depth, (1024, 512), interpolation=cv2.INTER_NEAREST)
    depth[~np.isfinite(depth)] = 0.
    depth[depth == 0] = -1.0
    return depth

def depth_read_stfd(filename: str):
    # loads depth map D from png file
    # and returns it as a numpy array,
    height, width = 512, 1024
    depth = cv2.imread(filename, cv2.IMREAD_ANYDEPTH).astype(np.float32) / 512.
    depth = cv2.resize(depth, (width, height), interpolation=cv2.INTER_NEAREST)

    # mask
    depth[0:int(height*0.15), :] = 0
    depth[height-int(height*0.15):height, :] = 0
...
def depth_evaluation(
    ...
    if metric_scale:
        ...
        dot_pred_gt = torch.nanmedian(ground_truth_depth)
        dot_pred_pred = torch.nanmedian(predicted_depth)
    ...
    # Reshape the depth_error_parity_map back to the original image shape
