...
from wid3r.utils.geometry import se3_inverse
import ipdb
...
def load_image_dk(filelist: List[str], width: int, height: int, verbose: bool = False):
    """
    Loads images from a directory or video, resizes them to a uniform size,
    then converts and stacks them into a single [N, 3, H, W] PyTorch tensor.
    """
    ...
    # --- 2. Resize images to the specified width and height ---
    tensor_list = []
    to_tensor_transform = tvf.ToTensor()
    for img_pil in sources:
        try:
            resized_img = img_pil.resize((width, height), Image.Resampling.LANCZOS)
            img_tensor = to_tensor_transform(resized_img)
            tensor_list.append(img_tensor)
        except Exception as e:
            print(f"Error processing an image: {e}")
    ...
    # --- 3. Stack the list of tensors into a single [N, 3, H, W] batch tensor ---
    return torch.stack(tensor_list, dim=0)
    

def infer_monodepth(file: str, camera, model: Wid3R, hydra_cfg: DictConfig):

    imgs = load_image_dk([file], width=hydra_cfg.width, height=hydra_cfg.height, verbose=hydra_cfg.verbose).to(hydra_cfg.device)

    dtype = torch.bfloat16 if torch.cuda.get_device_capability()[0] >= 8 else torch.float16
    with torch.no_grad():
        with torch.amp.autocast(hydra_cfg.device, dtype=dtype):
            pred = model(imgs[None], cameras=camera)

    points = pred['local_points'][0]         # (1, h_14, w_14, 3)
    # depth_map = points[0, ..., -1].detach()  # (h_14, w_14)
    # return depth_map  # torch.Tensor
    distance_map = pred["local_points"].norm(dim=-1).detach()[0, 0]
    return distance_map  # torch.Tensor


def infer_videodepth(filelist: str, model: Wid3R, hydra_cfg: DictConfig):

    imgs = load_image_dk(filelist, width=hydra_cfg.width, height=hydra_cfg.height, verbose=hydra_cfg.verbose).to(hydra_cfg.device)
...
def infer_cameras_w2c(filelist: str, cameras, model: Wid3R, hydra_cfg: DictConfig):
    ...
    with torch.no_grad():
        with torch.amp.autocast(hydra_cfg.device, dtype=dtype):
            pred = model(imgs[None], cameras=cameras)
...
def infer_cameras_c2w(filelist: str, cameras, model: Wid3R, hydra_cfg: DictConfig):
    ...
    return poses_c2w_all[0], pred


def infer_mv_pointclouds(filelist: str, cameras, model: Wid3R, hydra_cfg: DictConfig, data_size: Tuple[int, int]):
